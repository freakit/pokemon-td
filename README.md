...
- **진화**:
  ...
  - 돌 진화: 불의 돌, 물의 돌, 천둥의 돌, 리프의 돌, 달의 돌, 빛의 돌
...
| Wave 11-20 | Gold ~ Diamond | 어려움 |
| Wave 21-30 | Diamond ~ Master | 매우 어려움 |
| Wave 31+ | Master ~ Legend | 극한 (메가진화 필수) |
...
6. **메가진화**: 웨이브 클리어 시 2% 확률로 메가스톤 획득
...
- **메가진화 전략**: 메가진화 가능한 포켓몬을 엔트리에 유지하여 메가스톤 획득 기회 증대
...
src/
├── api/
│   └── pokeapi.ts              # PokeAPI 연동 (1-9세대, 명중률)
├── components/
│   ├── Game/
...
│       ├── WaveEndPicker.tsx  # 웨이브 보상 (메가스톤)
│       ├── Pokedex.tsx        # 도감
│       ├── Achievements.tsx   # 업적
│       └── Settings.tsx       # 설정
├── data/
│   ├── evolution.ts           # 진화 데이터 (1-9세대, 메가진화)
│   ├── maps.ts               # 맵 데이터
│   └── achievements.ts       # 업적 데이터
├── game/
│   ├── GameManager.ts        # 게임 로직 (메가스톤 드랍, 명중률)
│   └── WaveSystem.ts         # 웨이브 시스템 (1-9세대)
├── services/
│   ├── SaveService.ts        # 저장/로드
│   └── SoundService.ts       # 사운드
├── store/
│   └── gameStore.ts          # Zustand 상태 관리 (고유 스탯 진화)
├── types/
│   └── game.ts               # 타입 정의 (메가스톤)
├── utils/
